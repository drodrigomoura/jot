...
@section('content')
<div class="mx-auto h-full flex justify-center bg-gray-300">
    <div class="w-96 bg-blue-900 rounded-lg shadow-xl p-6">
        ...
        <form method="POST" action="{{ route('password.update') }}" class="pt-8">
            ...
            <div class="relative">
                ...
                <div class="">
                    <input id="email" type="email" class="pt-8 w-full rounded p-3 bg-blue-800 text-gray-100 outline-none focus:bg-blue-700" name="email" value="{{ $email ?? old('email') }}" autocomplete="email" autofocus>

                    @error('email')
                    <span class="text-red-600 text-sm pt-1 invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                    @enderror
                </div>
            </div>
